...
export const Main: FunctionComponent<IMainProps> = (props: IMainProps) => {
    ...
    // Function that determines when to send the GET Request to retrieve TodoList
    const getTodoListRequestHandler = (event: React.KeyboardEvent<HTMLInputElement>) => {
        if (event.key === 'Enter') {
            props.getTodoListRequest();
        }
    };
    ...
    return (
        <div className='root-container'>
            ...
            <input
                ...
                onKeyPress={e => getTodoListRequestHandler(e)}
            />
        </div>
    );
};
